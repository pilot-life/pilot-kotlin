 ...
 *
 * Previous JVM-only versions extended `java.io.IOException` to work around
 * an OkHttp interceptor-chain quirk where non-IOException throws got
 * wrapped in `IOException("canceled due to ...")`. The Ktor-based KMP
 * implementation doesn't have that quirk, so PartnerException extends
 * `Exception` directly — KMP-friendly. The only consumer impact is JVM
 * partners who explicitly `catch (e: IOException)` instead of
