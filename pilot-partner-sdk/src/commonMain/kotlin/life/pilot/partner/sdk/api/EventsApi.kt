...
class EventsApi internal constructor(private val http: HttpClient) {
    ...
    suspend fun list(
        ...
        cursor: String? = null,
        limit: Int? = null,
    ): EventList = http.partnerGet("events") {
    ...
    suspend fun get(eventUuid: String): EventDetail =
        http.partnerGet("events/$eventUuid").body()
    ...
    ): RtaCreateResponse = http.partnerPost("events/$eventUuid/rta") {
    ...
    ): RegistrationCreateResponse = http.partnerPost("events/$eventUuid/registrations") {
        headers { append("Idempotency-Key", idempotencyKey) }
        contentType(ContentType.Application.Json)
        setBody(body)
    }.body()
    ...
    suspend fun inventory(
        ...
        val response: HttpResponse = http.partnerGet("events/$eventUuid/inventory") {
